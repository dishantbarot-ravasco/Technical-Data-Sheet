...
from apps.services.pdf_service import build_tds_doc_data, tds_filename
...
def generate_pdf(request, tds_id):
    ...
    # See tds_filename()'s docstring: doc number / customer name / tds_number
    # base, plus a "_rev_NN" suffix only once the record has actually been
    # edited at least once.
    filename = tds_filename(tds_record) if tds_record else f"{doc.tds_number}.pdf"
